...
void Models::Shader(Vec3 *point, WindowManager &window,
  ...
  int temp = _currentTexture % material.size();
  Vec3 def(0, 0, 0);
  Vec3 normal(0, 1, 0);
  Vec3 lightDir = Normalize(Sub(window._lightData, *point));
  Vec3 viewDir = Normalize(Sub(window._viewPos, *point));
  Vec3 ambient = Multiply(material[temp].Ka, window._lightColor);
  Vec3 diffIntensity = Max(Dot(normal, lightDir), def);
  Vec3 diffHelper = Multiply(material[temp].Kd, window._lightColor);
  Vec3 diffuse = Multiply(diffHelper, diffIntensity);
  Vec3 reflectDir = Reflect(Sub(def, lightDir), normal);
  Vec3 specIntensity =
      Pow(Max(Dot(viewDir, reflectDir), def), material[temp].Ns);
  Vec3 specular =
      Multiply(Multiply(material[temp].Ks, window._lightColor), specIntensity);
  Vec3 result = Add(ambient, Add(diffuse, specular));
  glColor4f(result.x, result.y, result.z, material[temp].d);
}
